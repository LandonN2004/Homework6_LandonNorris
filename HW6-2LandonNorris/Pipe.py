...
class Pipe():
    ...
    def FrictionFactor(self):
        ...
        sig = 0.2 * mean
        return max(rnd.normalvariate(mean, sig), 1e-6)
    ...
    def frictionHeadLoss(self):  # calculate headloss through a section of pipe in m of fluid
        ...
        hl = ff*(self.length/self.d)*(self.V()**2)/(2*g)
        return hl
    ...
    def printPipeFlowRate(self):
        print('The flow in segment {} is {:0.4f} m^3/s ({:0.2f} L/s)'.format(self.Name(),self.Q/1000.0,self.Q))
